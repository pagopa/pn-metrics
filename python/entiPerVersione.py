...
# === 1) Percorso file CSV ===
csv_path = Path(r"C:\Users\CA455PE\OneDrive - EY\Desktop\Progetti\PagoPa\SEND\entiperversione\dati.csv")
...
required = {"p_year", "p_month", "version", "occurrences"}
...
occurrences_clean = (
    df["occurrences"].astype(str).str.strip()
      .str.replace(".", "", regex=False)
      .str.replace(",", ".", regex=False)
)
df["occurrences"] = pd.to_numeric(occurrences_clean, errors="coerce")
...
if (~mask_ok).any():
    ...
    print(df.loc[~mask_ok, ["p_year", "p_month", "version", "occurrences"]])
...
agg = df.groupby(["Periodo", "version"], as_index=False)["occurrences"].sum()
pivot = agg.pivot(index="Periodo", columns="version", values="occurrences").sort_index()
...
ax.set_title("Enti per versione nel tempo")
ax.set_xlabel("Periodo (Anno-Mese)")
ax.set_ylabel("Numero enti per versione")
...
# === 8) Salvataggio ===
output_png = csv_path.with_name("EntiPerVersione.png")
